...
data_20mm = data_20mm.drop(columns=["ODB Name", "Step", "Part Instance Name", "Section Name", "Material Name", "Section Point"])

data_5mm["U-U2"] = -data_5mm["U-U2"]
data_10mm["U-U2"] = -data_10mm["U-U2"]
data_20mm["U-U2"] = -data_20mm["U-U2"]

data_5mm["RF-RF2"] = abs(data_5mm["RF-RF2"])
data_10mm["RF-RF2"] = abs(data_10mm["RF-RF2"])
data_20mm["RF-RF2"] = abs(data_20mm["RF-RF2"])

funct = lambda x: int(float(x[-5:]))
data_5mm['time'] = data_5mm['Frame'].apply(funct)
data_10mm['time'] = data_10mm['Frame'].apply(funct)
data_20mm['time'] = data_20mm['Frame'].apply(funct)

data_5mm['Force'] = data_5mm.groupby('time')['RF-RF2'].transform('sum')
data_10mm['Force'] = data_10mm.groupby('time')['RF-RF2'].transform('sum')
data_20mm['Force'] = data_20mm.groupby('time')['RF-RF2'].transform('sum')

data_5mm['Force'] = data_5mm['Force'] * 1200 / 6708.888564
data_10mm['Force'] = data_10mm['Force'] * 2190 / 13109.401429
data_20mm['Force'] = data_20mm['Force'] * 2610 / 16829.253894


data_5mm = data_5mm.sort_values(by='time', kind='stable')
data_10mm = data_10mm.sort_values(by='time', kind='stable')
data_20mm = data_20mm.sort_values(by='time', kind='stable')
...
time_5mm = non_zero_ac_yield_5mm['time']
time_10mm = non_zero_ac_yield_10mm['time']
time_20mm = non_zero_ac_yield_20mm['time']
...
node_5mm = node_5mm[node_5mm['U-U2'] <= 8.0]
node_10mm = node_10mm[node_10mm['U-U2'] <= 8.0]
node_20mm = node_20mm[node_20mm['U-U2'] <= 8.0]


print("5mm")
print(f"The plastification occurs at time {time_5mm} for force: {force_5mm} in node {node_label_5mm} at coordinates ({x_5mm}, {y_5mm}, {z_5mm})")
print(f"The corresponding displacement is: {non_zero_ac_yield_5mm['U-U2']} mm")
print("-" * 50)
print("10mm")
print(f"The plastification occurs at time {time_10mm} for force: {force_10mm} in node {node_label_10mm} at coordinates ({x_10mm}, {y_10mm}, {z_10mm})")
print(f"The corresponding displacement is: {non_zero_ac_yield_10mm['U-U2']} mm")
print("-" * 50)
print("20mm")
print(f"The plastification occurs at time {time_20mm} for force: {force_20mm} in node {node_label_20mm} at coordinates ({x_20mm}, {y_20mm}, {z_20mm})")
...
plt.ylabel('Force [N]')
plt.title('Force-displacement curve considering softening')
plt.legend()
plt.grid()
plt.tight_layout()
